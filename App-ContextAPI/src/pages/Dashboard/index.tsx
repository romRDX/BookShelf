...
console.log('SheetGo - Bookshelf / CONTEXT-API');

const Dashboard: React.FC<Props> = ({books, location, dispatch}) => {
  const [availableBooks, setAvailableBooks] = useState<IBook[]>([{ author: 'rdx'} as IBook]);
  ...
  const { contextBooks, createBook } = useBooks();

  // Updates availableBooks state whenever there is a change on global state
  useEffect(()=> {
    setAvailableBooks(contextBooks);
  }, [contextBooks, setAvailableBooks]);
